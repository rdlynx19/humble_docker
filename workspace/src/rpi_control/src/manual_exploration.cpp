...
{	
	...
	if(static_cast<int>(status_msg->nav_state) == static_cast<int>(VehicleStatus::NAVIGATION_STATE_OFFBOARD))
	{
		RCLCPP_INFO_ONCE(this->get_logger(), "Successfully switched to offboard mode");			
	}
}
...
{
	RCLCPP_INFO(this->get_logger(), "Entered the callback!");
	if (arm_disarm_request->flight_request == "arm")
	{
		RCLCPP_INFO(this->get_logger(), "Just sending request");
		arm();
